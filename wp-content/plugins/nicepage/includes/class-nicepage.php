    ...
    private static function _processBlogControl($content) {
        $content = preg_replace_callback(
            ...
            function ($blogMatch) {
                ...
                $blogHtml .= GridHelper::buildGridAutoRowsStyles($blogGridProps, count($blog_control_query->posts));
    ...
    private static function _processPost($content, $type='full') {
        ...
        if (method_exists($blog_control_query, 'have_posts')) {
            ...
            while($blog_control_query->have_posts()) :
                ...
                Nicepage::$_postId = Nicepage::$_post->ID;
                if (strpos(Nicepage::$_post->post_title, '$') !== false) {
                    Nicepage::$_post->post_title = str_replace('$', '[[$]]', Nicepage::$_post->post_title);
                }
                if (strpos(Nicepage::$_post->post_content, '$') !== false) {
                    Nicepage::$_post->post_content = str_replace('$', '[[$]]', Nicepage::$_post->post_content);
                }
